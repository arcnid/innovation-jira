...
const formSchema = z.object({
  ...
  impact: z.string({ required_error: "Please select the potential impact." }),
  timeframe: z.string({
  ...
  targetUsers: z
    .string()
    .min(10, { message: "Target users must be at least 10 characters." }),
  businessCase: z
    .string()
    .min(50, { message: "Business case must be at least 50 characters." }),
  risks: z
    .string()
    .min(20, { message: "Risks must be at least 20 characters." }),
...
export default function IdeaSubmissionPage() {
  ...
  // New state to store the file binary data
  const [fileBinaryData, setFileBinaryData] = useState<ArrayBuffer | null>(
    null
  );
  ...
  // Recalculate progress whenever any form field changes
  useEffect(() => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onload = function (event) {
        setFileBinaryData(event.target?.result as ArrayBuffer);
      };
      ...
      <main className="flex-1 py-8">
        <div className="w-full max-w-5xl px-4 md:px-8 mx-auto">
          ...
          <div className="grid gap-8 md:grid-cols-[300px_1fr]">
            ...
            <div>
              <Form {...form}>
                {/* Removed onSubmit from form element */}
