...
export default function DepartmentProjectsPage() {
		...
		fetch("/api/projects")
			...
			.then((data: any) => {
				...
				const uniqueDepts = Array.from(
					new Set(activeProjects.map((p) => p.department))
				);
	...
	const filteredProjects =
		selectedDepartment === "all"
			? projects
			: projects.filter(
					(project: any) => project.department === selectedDepartment
			  );
				...
				<div className="w-full max-w-5xl px-4 md:px-8 mx-auto">
					<div className="mb-8">
						...
						<p className="mt-2 text-lg text-muted-foreground">
							A high-level view of current projects across all departments
						</p>
					</div>
